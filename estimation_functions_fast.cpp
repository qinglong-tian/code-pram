...
NumericMatrix Compute_IF_Linear_Solving_CPP(NumericVector beta_hat, double p00, double p11, NumericMatrix pinv, NumericVector prob_x_star_1_y, NumericVector yVec, NumericVector xStar)
{
  int num = yVec.length();
  double pxs1y, pxs0y;
  double px0y, px1y;
  ...
  for (int i=1; i<num; i++)
  {
    ...
    UMat = Compute_U_Linear_CPP(beta_hat, yVec(i));
    IFMat(i,0) = UMat(0,0)*px0yxs+UMat(1,0)*px1yxs;
    IFMat(i,1) = UMat(0,1)*px0yxs+UMat(1,1)*px1yxs;
  }
  return IFMat;
...
double Compute_IF_Solving_Linear_Sum_CPP(NumericVector beta_hat, double p00, double p11, NumericMatrix pinv, NumericVector prob_x_star_1_y, NumericVector yVec, NumericVector xStar)
{
  NumericMatrix nm = Compute_IF_Linear_Solving_CPP(beta_hat, p00, p11, pinv, prob_x_star_1_y, yVec, xStar);
  int num_of_row = nm.nrow();
  double tmp1 = 0, tmp2 = 0;
  for (int i=0; i< num_of_row; i++)
  {
